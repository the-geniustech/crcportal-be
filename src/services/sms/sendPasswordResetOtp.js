...
async function verifyGenericFallbackDelivery(sendResult) {
  ...
  console.warn("Termii generic password-reset OTP fallback accepted", {
    messageId,
  ...
  try {
    ...
    if (!historyEntry) {
      throw new AppError(
        "Termii accepted the fallback SMS request, but delivery could not be confirmed yet. Because the DND route is inactive on this account, password-reset OTP SMS cannot be treated as delivered until Termii activates the route.",
        502,
      );
    }
    ...
    if (isDeliveredSmsStatus(status)) {
      console.warn("Termii generic password-reset OTP fallback delivered", {
        messageId,
        status,
      });
      return historyEntry;
    ...
    throw new AppError(
      `Termii accepted the fallback SMS request, but delivery is still in status \"${status || "pending"}\". Because the DND route is inactive on this account, password-reset OTP SMS cannot be treated as delivered yet.`,
      502,
    );
  } catch (error) {
    if (
